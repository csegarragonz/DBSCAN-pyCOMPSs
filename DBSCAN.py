...
def normalizeData(dataFile):
    ...
    normData = np.where(np.max(dataset, axis=0)==0, dataset, 
...
def partitionSpace(dataset, fragSize, epsilon):
    ...
    fragVec = [[np.max(np.min(dataset, axis=0)[i]-epsilon,0), 
        np.mean(dataset,axis=0)[i], np.max(dataset, axis=0)[i]] for i in 
        range(dim)]
    ...
    for square in fragData:
        ...
        for i in range(dim):
            pos=square%size
            a = [[j,x-fragVec[i][pos]] for j,x in enumerate(fragVec[i]) if 
                abs(x-fragVec[i][pos]) < epsilon]
            b = [[j,x-fragVec[i][pos+1]] for j,x in enumerate(fragVec[i]) if 
                abs(x-fragVec[i][pos+1]) < epsilon]
...
def partialScan(corePoints, square, epsilon, minPoints, fragData, fragSize,
    numParts, rangeToEps):
    """
    Looks for all the core points (over minPoints neighbours) inside a certain 
    square.
    ...
    :param rangeToEps:  for each square, number of neighbors until epsilon 
    distance. 
    """
...
def syncClusters(clusters, epsilon, numParts):
    """
    Returns a matrix of booleans. Pos [i,j]=1 <=> clusters -i and -j should be 
    merged.
    ...
    if not len(clusters)%numComp == 0:
        for i in range(numComp - len(clusters)%numComp): 
            clusters.append(Cluster())
    enable = [[[] for z in range(len(clusters)/numComp)] for w in 
        range(len(clusters)/numComp)]
...
def syncTask(enable, hosts, visits, epsilon):
    """
    Given two lists, checks wether the distance between the two sets is less 
    than epsilon.
    """
    for host in hosts:
        for visit in visits:
...
def neighExpansion(clustPoints, clust, fragData, fragSize, rangeToEps, epsilon):
    """
    Given a cluster of core points, returns all the points lying in the squares
    reachable.
...
def update(clusters, possibleClusters, returnCluster):
    ...
    :param returnCluster:       indicates wether the result must mantain the 
    cluster structure.
...
def DBSCAN(dataFile, fragSize, epsilon, minPoints, numParts):
    ...
    :param fragData:    dict with the partitioned space and the points
    classified.
    ...
    for i, (square,value) in enumerate(fragData.viewitems()):
        partialScan(corePMatrix[i], square, epsilon, minPoints, fragData,
    ...
    iniP = time.time()
    clusters = [clust for rows in clusters for squares in rows for clust in 
        squares] 
    ...
    defClusters = expandCluster(halfDefClusters, fragData, epsilon, minPoints, 
        fragSize, numParts, rangeToEps)
    end = time.time()
