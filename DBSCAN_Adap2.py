...
def DBSCAN(dataFile, fragSize, epsilon, minPoints, numParts):
    ...
    plotCluster(clusters, "initialProposal", fragVec)
    ...
    plotCluster(halfDefClusters, "firstMerge", fragVec)
...
def plotCluster(defCluster, figName, fragVec):
	"""
	...
	"""
        ...
        for i,key in enumerate(defCluster):
                ax.scatter([p[0] for p in key.points], [p[1] for p in key.points], color=colours[i], s=1)
