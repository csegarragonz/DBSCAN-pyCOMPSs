...
def normalizeData(dataFile):
    """
    Given a dataset, divide each dimension by its maximum obtaining data in
     the range [0,1]^n
...
def expandCluster(point, neighborPts, c, epsilon, minPoints):
    ...
    for p in neighborPts:
        if not visited[str(p)]:
            visited[str(p)] = True
            neighborPts2 = regionQuery(dataset, p, epsilon)
...
def regionQuery(dataset, point, epsilon):
    ...
    """
    distance = [np.linalg.norm(point-p) for p in dataset]
    neighborPts = [p for i, p in enumerate(dataset) if (p-point).all() and distance[i]
        < epsilon]
    return neighborPts
...
if __name__ == "__main__":
    ...
    if plot:
        ...
        fig, ax = plt.subplots()
        for c in cluster:
            ax.scatter([p[0] for p in cluster[c]], [p[1] for p in cluster[c]], 
                color=colours[c])
        plt.show()
