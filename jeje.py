...
def sync_clusters(data, adj_mat, epsilon, coord, neigh_sq_loc, quocient,
    ...
    count_ent = 0
    acc = 0
    print "Time before enering loops: "+str(tbl-start) 
    for num1, point1 in enumerate(data.value[0]):
        ...
        if current_clust_id > -1:
            ...
            acc += tmp_time-tb2l
            tb2l = tmp_time
    print "We entered: "+str(count_ent)+" times the inner loop."
    if count_ent:
        print "On average inner loop lasted: "+str(float(acc)/count_ent)
    print "The outer loop lasted: " + str(float(time.time()-tbl))
    print "The whole task lasted: " + str(float(time.time()-start))
    return adj_mat_copy
